...
class ArregloClaves
{
	public:
		...
		void InsertarClave(pNodoB Clave, int Numero);
		int ObtenerClave(int Numero);
		pNodoB ObtenerApuntadorClave(int Numero);
		
		
	private:
		
		pNodoB arregloClaves[5];
	
	friend class Pagina;	
};

void ArregloClaves::InsertarClave(pNodoB Clave, int Numero)
{
...
pNodoB ArregloClaves::ObtenerApuntadorClave(int Numero)
{
	pNodoB Clave;
	Clave = arregloClaves[Numero];
	return Clave;
...
ApuntadorPagina ArbolB::InsertarB(ApuntadorPagina Raiz, int Numero, string pIdCliente, string pNombre, string pApellido, string pCiudad, string pTelefono)
{
	...
	if(Raiz->EmpujarArriba == true){
		P = new Pagina();
		P->cuenta = 1;
		pNodoB Auxiliar = new NodoArbolB(Raiz->X,pIdCliente, pNombre, pApellido, pCiudad, pTelefono);
...
ApuntadorPagina ArbolB::MeterHojaB(ApuntadorPagina Raiz, string pIdCliente, string pNombre, string pApellido, string pCiudad, string pTelefono)
{
	...
	pNodoB X = new NodoArbolB(Raiz->X, pIdCliente, pNombre, pApellido, pCiudad, pTelefono);
...
void PilaB::Push(pNodoB v)
{
   if(Vacia()) {
      plistaB = v;
   }else{
    NodoArbolB *aux = plistaB;
    while(aux->siguiente){
        aux = aux->siguiente;
    }
    aux->siguiente = v;
...
pNodoB PilaB::Pop()
{
    NodoArbolB *borrar = NULL;
    NodoArbolB *salida = NULL;
    if(Vacia()){
